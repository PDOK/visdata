...
ogr2ogr -f GeoJSON /data/geojson/infra_500.geojson -s_srs EPSG:28992 -t_srs EPSG:4326 PG:"host=postgis port=5432 user=pdok_owner dbname=visdata password=Ip8pVdIM3eJKzs4aSU1ylfiJaubBRrKu" -sql "SELECT lod1, lod2, name, z_index, original_id, geom FROM visdata.infrastructure_line WHERE original_source='TOP500NL' and lod2 NOT IN ('artiary', 'local')" -lco COORDINATE_PRECISION=6

ogr2ogr -f GeoJSON /data/geojson/infra_1000.geojson -s_srs EPSG:28992 -t_srs EPSG:4326 PG:"host=postgis port=5432 user=pdok_owner dbname=visdata password=Ip8pVdIM3eJKzs4aSU1ylfiJaubBRrKu" -sql "SELECT lod1, lod2, name, z_index, original_id, geom FROM visdata.infrastructure_line WHERE original_source='TOP1000NL' and lod2 NOT IN ('artiary', 'local')" -lco COORDINATE_PRECISION=6
